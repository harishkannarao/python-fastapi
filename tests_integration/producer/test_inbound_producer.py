...
@pytest.fixture
def mock_publish_to_inbound_from_retry(mocker: MockerFixture) -> AsyncMock:
    mock: AsyncMock = mocker.patch("app.consumer.retry_consumer.publish_to_inbound")
    return mock
...
def test_publish_inbound_message_sent_to_retry_dlq_if_publish_to_inbound_fails_on_retry(
    enable_test_components: Settings,
    mock_publish_to_outbound: AsyncMock,
    mock_publish_to_inbound_from_retry: AsyncMock,
    test_client: TestClient,
    captured_logs: list[MutableMapping[str, Any]],
):
    mock_publish_to_outbound.side_effect = ValueError("Dummy failure")
    mock_publish_to_inbound_from_retry.side_effect = ValueError("Another Dummy failure")
    ...
    for attempt in Retrying(
        ...
        with attempt:
            assert_that(mock_publish_to_outbound.call_count).is_equal_to(1)
            assert_that(mock_publish_to_inbound_from_retry.call_count).is_equal_to(1)

            assert_that(len(captured_logs)).is_greater_than(0)
            retry_dlq_logs = list(
                filter(
                    lambda entry: str(entry["event"]).startswith(
                        "Received inbound retry dlq message"
                    ),
                    captured_logs,
                )
            )
            assert_that(retry_dlq_logs).is_length(1)
            retry_headers: HeadersType = retry_dlq_logs[0]["headers"]
            assert_that(retry_headers.get("message_id")).is_equal_to(
                headers.get("message_id")
            )
