...
@pytest.mark.asyncio
async def test_sample_read_all_with_offset_and_limit(get_database: Database):
    ...
    sample_id_1: UUID = await sample_sql_dao.create_sample(
        get_database, create_request_1
    )
    sample_1 = await sample_sql_dao.read_sample_by_id(get_database, sample_id_1)
    ...
    sample_id_2: UUID = await sample_sql_dao.create_sample(
        get_database, create_request_2
    )
    sample_2 = await sample_sql_dao.read_sample_by_id(get_database, sample_id_2)
    ...
    sample_id_3: UUID = await sample_sql_dao.create_sample(
        get_database, create_request_3
    )
...
@pytest.mark.asyncio
async def test_sample_delete_all(get_database: Database):
    create_request_1: SampleCreate = SampleCreate(
        username=f"usr-{uuid.uuid4()}",
        bool_field=True,
        float_field=1,
    ...
    assert_that(
        await sample_sql_dao.read_sample_by_id(get_database, sample_id_1)
    ).is_none()
    assert_that(
        await sample_sql_dao.read_sample_by_id(get_database, sample_id_2)
    ).is_none()
    assert_that(
        await sample_sql_dao.read_sample_by_id(get_database, sample_id_3)
    ).is_none()


@pytest.mark.asyncio
async def test_sample_create_and_read(get_database: Database):
    ...
    sample_id: UUID = await sample_sql_dao.create_sample(get_database, create_request)

    read_by_id: Sample = await sample_sql_dao.read_sample_by_id(get_database, sample_id)
...
@pytest.mark.asyncio
async def test_sample_delete(get_database: Database):
    create_request: SampleCreate = SampleCreate(
        username=f"usr-{uuid.uuid4()}",
        bool_field=True,
        float_field=2.1,
    ...
    deleted_id = await sample_sql_dao.delete_by_id(get_database, sample_id)

    assert_that(deleted_id).is_equal_to(sample_id)

    read_after_delete = await sample_sql_dao.read_sample_by_id(get_database, sample_id)
...
async def test_sample_update_and_read(get_database: Database):
    ...
    created_sample: Sample = await sample_sql_dao.read_sample_by_id(
    ...
    update_ids: list[UUID] = await sample_sql_dao.update_sample(
        get_database, update_request
    )
    ...
    updated_sample: Sample = await sample_sql_dao.read_sample_by_id(
        get_database, sample_id
    )
