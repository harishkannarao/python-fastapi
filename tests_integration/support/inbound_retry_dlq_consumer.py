...
async def process_inbound_retry_dlq_message_task(message: AbstractIncomingMessage):
    ...
    async with message.process():  # Automatically ACKs if no exception occurs
        ...
        logger.info(
            f"Received inbound retry dlq message {payload_string}",
            payload=payload_string,
            headers=headers,
        )
        samples = [Sample(**item) for item in json.loads(payload_string)]
        logger.info(
            f"Processed inbound retry dlq message {payload_string}",
            headers=headers,
            samples=jsonable_encoder(samples),
        )


async def start_inbound_retry_dlq_consumer() -> Task[str]:
    logger = structlog.get_logger()
    ...
    queue = await channel.get_queue(f"{settings.app_rabbit_inbound_retry_queue}.dlq")
    # Fire-and-forget the consumer loop into the background loop
    task: Task[str] = asyncio.create_task(
        queue.consume(process_inbound_retry_dlq_message_task)
    )
    logger.info("Started inbound retry dlq message consumer")
    return task
