...
async def process_outbound_message_task(message: AbstractIncomingMessage):
    ...
    async with message.process():  # Automatically ACKs if no exception occurs
        ...
        logger.info(
            f"Received outbound message {payload_string}",
            payload=payload_string,
            headers=headers,
        )
        samples = [Sample(**item) for item in json.loads(payload_string)]
        logger.info(
            f"Processed outbound message {payload_string}",
            headers=headers,
            samples=jsonable_encoder(samples),
        )


async def start_outbound_consumer() -> Task[str]:
    logger = structlog.get_logger()
    ...
    queue = await channel.get_queue(settings.app_rabbit_outbound_queue)
    # Fire-and-forget the consumer loop into the background loop
    task: Task[str] = asyncio.create_task(queue.consume(process_outbound_message_task))
    logger.info("Started outbound message consumer")
    return task
