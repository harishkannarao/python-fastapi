...
from fastapi import UploadFile, File, Form, HTTPException, Path as PathParam
...
async def handle_form(
    ...
    if files:
        for file in files:
            ...
            target_path: str = os.path.join(settings.app_file_upload_path, file.filename)

            # Asynchronous streaming write
            async with aiofiles.open(target_path, 'wb') as out_file:
                while content := await file.read(1024 * 16):  # 16KB chunks like your Java code
                    await out_file.write(content)

            await file.close()

    return RedirectResponse(url="/context/static/sample_form_submit_success.html", status_code=status.HTTP_303_SEE_OTHER)
